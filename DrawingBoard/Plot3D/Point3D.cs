...
namespace DrawingBoard.Plot3D
{
    ...
    public struct Point3D : IEquatable<Point3D>
    {
        ...
        /// </summary>
        /// <param name="cameraregion">The location to base perspective calculations on.</param>
        ...
        /// </remarks>
        public PointF GetScreenregion(Point3D cameraregion)
        {
            PointF returnValue = new PointF();

            returnValue.X = (float)
                (((((double)this.X - (double)cameraregion.X) 
                * (-1 * (double)cameraregion.Z)) 
                / ((double)this.Z - (double)cameraregion.Z))
                + (double)cameraregion.X);

            returnValue.Y = (float)
                (((((double)this.Y - (double)cameraregion.Y) 
                * (-1 * (double)cameraregion.Z)) 
                / ((double)this.Z - (double)cameraregion.Z)) 
                + (double)cameraregion.Y);

            if (float.IsInfinity(returnValue.X) || float.IsNaN(returnValue.X)
                || float.IsInfinity(returnValue.Y) || float.IsNaN(returnValue.Y))
            {
                throw new ArgumentOutOfRangeException("cameraregion", string.Format("The point cannot be represented with the specified camera region because the X or Y coordinate of the projected point was either Infinity or NaN." + Environment.NewLine + "Object Point: {0}" + Environment.NewLine + "Camera Point {1}", this.ToString(), cameraregion.ToString()));
            }
