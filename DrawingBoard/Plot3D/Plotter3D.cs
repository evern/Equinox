...
namespace DrawingBoard.Plot3D
{
    /// <summary>
    /// Plots points and draws lines in 3D space.
    /// source : http://www.codeproject.com/KB/GDI-plus/Plot3D.aspx
    /// </summary>
    public class Plotter3D : IDisposable
    {
        ...
        /// </summary>
        public Plotter3D() : this(new Pen(Color.Black)) {}

        /// <summary>
        /// Instantiates a new Plotter.
        /// </summary>
        /// <param name="pen">The pen we want to use to draw on the canvas.</param>
        public Plotter3D(Pen pen) : this( pen, new Point3D(-30, 0, -600)) {}

        /// <summary>
        /// Instantiates a new Plotter.
        /// </summary>
        /// <param name="cameraLocation">The location of the camera that we use to calculate perspective.</param>
        public Plotter3D(Point3D cameraLocation) : this(new Pen(Color.Black), cameraLocation) {}
        ...
        /// <param name="pen">The pen we want to use to draw on the canvas.</param>
        /// <param name="cameraLocation">The location of the camera that we use to calculate perspective.</param>
        public Plotter3D(Pen pen, Point3D cameraLocation)
        {
           // this.canvas = canvas;
            this.pen = pen;
        ...
        /// change the region of the pen, but won't draw anything.
        /// </summary>
        ...
        public PointF[] Forward(double distance)
        {
            ...
            if (IsPenDown)
            {
                points[0] = oldLocation.GetScreenregion(cameraLocation);
                points[1] = this.Location.GetScreenregion(cameraLocation);

                ExpandBoundingBox(oldLocation.GetScreenregion(this.CameraLocation));
                ExpandBoundingBox(this.Location.GetScreenregion(this.CameraLocation));
            }
            return points;
        }
        ...
        /// to the spirit of this library.  This library uses relative regioning, which 
        /// allows you to define an object relatively, then move or rotate it however you like,
        /// and that all falls apart as soon as you start using absolute regioning.  Nevertheless,
        /// there are some times when it's useful to be able to draw a line to an absolute region,
        /// so there are times when this method is handy.  But use it sparingly.
        /// </remarks>
        public PointF[] MoveTo(Point3D newLocation)
        {
            Point3D oldLocation = location;
            ...
            if (IsPenDown)
            {
                ...
                //canvas.DrawLine(pen, oldLocation.GetScreenregion(cameraLocation), this.Location.GetScreenregion(cameraLocation));
